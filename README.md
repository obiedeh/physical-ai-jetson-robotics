...
| Rover SLAM measurement harness: session recorder, pose log, operator marks, drift and kidnap metrics | **implemented**; first session recorded on the Orin (stationary, below), lap and kidnap metrics not yet exercised on hardware | 2026-09-16 | Jetson Orin NX | [`scripts/jetson/slam_session.py`](scripts/jetson/slam_session.py), [`slam/metrics.py`](slam/metrics.py), [`docs/rosmaster_m3pro/SLAM_FIRST_RUN_PROTOCOL.md`](docs/rosmaster_m3pro/SLAM_FIRST_RUN_PROTOCOL.md) |
| Vendor SLAM Toolbox stack on the Orin, robot stationary: resource cost only | **measured**, 301 s recording; board input power VDD_IN p50 11,061 mW, slam_toolbox RSS peak 647 MB, 2,946 MB RAM available at minimum, 65.0 C peak zone temperature, no throttling suspected; no motion, so no map-quality or drift number | 2026-09-16 | Jetson Orin NX, MAXN_SUPER, desktop session active | [`reports/slam/sessions/2026-09-16_stationary/session.json`](reports/slam/sessions/2026-09-16_stationary/session.json), [`provenance.json`](reports/slam/sessions/2026-09-16_stationary/provenance.json) |
