...
| Rover SLAM measurement harness: session recorder, pose log, operator marks, drift and kidnap metrics | **scaffold**; unit-tested offline, never run on the Orin | | | [`scripts/jetson/slam_session.py`](scripts/jetson/slam_session.py), [`slam/metrics.py`](slam/metrics.py), [`docs/rosmaster_m3pro/SLAM_FIRST_RUN_PROTOCOL.md`](docs/rosmaster_m3pro/SLAM_FIRST_RUN_PROTOCOL.md) |
