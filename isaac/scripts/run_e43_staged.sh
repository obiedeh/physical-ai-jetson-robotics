...
LOG=$R/reports/logs/2026-08-06/e43_staged.log
PY="$HOME/.venv/isaacsim5/bin/python"
THOR=oedeh@192.168.1.170
CKROOT=/home/oedeh/models/gr00t_synria/gr00t_synria_thor_v3_visonly_cont/gr00t_synria_thor_v3_visonly_cont
rm -f $R/reports/e43_staged.DONE
exec >> "$LOG" 2>&1
echo "=== staged wait for screen $(date -Is)"
for i in $(seq 1 240); do [ -f $R/reports/e43_screen.DONE ] && break; sleep 15; done
...
for N in 10000; do
  ...
  ( cd $R && PYTHONUNBUFFERED=1 "$PY" -u isaac/scripts/eval_gr00t_sequence.py \
      ...
      > $R/reports/logs/2026-08-06/e43_staged_ckpt$N.log 2>&1 )
  grep -E "grasp:|carry5s:|dock:|cycle:" $R/reports/logs/2026-08-06/e43_staged_ckpt$N.log | tail -4
...
touch $R/reports/e43_staged.DONE
