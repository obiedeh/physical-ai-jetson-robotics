#!/usr/bin/env bash
# E45: fresh visual-only, one 40k-step schedule (time-parity campaign).
# Smoke-validated config: batch 8 (~80 GB), eager attention, unfrozen
# LLM + visual backbone, 2.38 s/step -> 10k steps ~ 6.7 h.
# Log: ~/gr00t_e45.log  Markers: ~/gr00t_e45.{DONE,FAIL}
set -u
LOG=~/gr00t_e45.log
rm -f ~/gr00t_e45.DONE ~/gr00t_e45.FAIL
exec >> "$LOG" 2>&1
...
OUT=~/models/gr00t_synria/gr00t_synria_thor_v5_vis40k
mkdir -p "$OUT"
~/.venv/gr00t/bin/python gr00t/experiment/launch_finetune.py \
  ...
  --output-dir "$OUT" \
  --experiment-name gr00t_synria_thor_v5_vis40k \
  --tune-visual \
  --global-batch-size 8 \
  --max-steps 40000 \
  --save-steps 1000 \
...
if [ $RC -eq 0 ]; then touch ~/gr00t_e45.DONE; else touch ~/gr00t_e45.FAIL; fi
