...
LOG=$R/reports/logs/2026-08-06/e45_staged.log
...
rm -f $R/reports/e45_staged.DONE
exec >> "$LOG" 2>&1
echo "=== staged wait for screen $(date -Is)"
for i in $(seq 1 240); do [ -f $R/reports/e45_screen.DONE ] && break; sleep 15; done
echo "=== staged start $(date -Is)"
for N in 36000 40000; do
  ...
  ( cd $R && PYTHONUNBUFFERED=1 "$PY" -u isaac/scripts/eval_gr00t_sequence.py \
      ...
      > $R/reports/logs/2026-08-06/e45_staged_ckpt$N.log 2>&1 )
  grep -E "grasp:|carry5s:|dock:|cycle:" $R/reports/logs/2026-08-06/e45_staged_ckpt$N.log | tail -4
...
touch $R/reports/e45_staged.DONE
